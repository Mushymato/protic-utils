...
<?php
function get_tradepost_rare($data, $include = array()){
	$out = array(
	...
	foreach($data as $d){
		...
		$mon = query_monster($d['monster_id']);
		if($mon){
...
function get_tradepost_farmable($data, $include = array()){
	$out = array(
...
$data = json_decode(file_get_contents("https://storage.googleapis.com/mirubot/protic/paddata/processed/{$rg}_exchange.json"), true);
$output_arr = $tf == 'rem' ? get_tradepost_rare($data, search_ids($input_str)) : get_tradepost_farmable($data, search_ids($input_str));?>
<p>Output</p>
