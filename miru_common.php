...
function get_card_grid($id, $region = 'jp', $right_side_table = false, $headings = false){
	...
	return array(
		...
		'shortcode' => $head . PHP_EOL . '<div class="col1">[pdxp id=' . $monster_no . ' r=' . $region . ']' . $stat1 . '</div>' . PHP_EOL . '<div class="col-cardinfo">' . PHP_EOL . '[' . $monster_no . ']<b>' . $atts[1] . htmlentities($data['name_na']) . ($data['name_na'] != $data['name_jp'] ? '<br/>' . youtube_link($data['name_jp']) : '') . '</b><br/><span class="card-type">' . PHP_EOL . $types[1] . '</span><br/>' . PHP_EOL . $awakes[1] . '<br/><br/>' . PHP_EOL . $stat2 . '<u>Active Skill:</u> <b>(' . $data['turn_max'] . ' &#10151; ' . $data['turn_min'] . ')</b> <span class="card-inherit">' . $inherit . '</span> <br/>' . htmlentities($data['as_desc_na'] . '<br/>' . PHP_EOL) . (strlen($data['ls_desc_na']) == 0 ? '' : '<br/><br/>' . PHP_EOL .'<u>Leader Skill:</u> <b>' . lead_mult($data) . '</b><br/>' . PHP_EOL . htmlentities($data['ls_desc_na']) ) . PHP_EOL . '</div>' . PHP_EOL . '</div>');
}
